...
def index():    
    ...
    covers = [f'../static/img/covers/{im}' for im in os.listdir('static/img/covers') if im.endswith('.jpg')]
    products = [f'../static/img/products/{p["id"]}/{p["images"][0]}' for p in getProducts() if len(p['images']) > 0]
...
def adminActivity():
    ...
    dailyUniqueVisits = database.getStats('dailyUniqueVisits')['data']
    sortedDailyUniqueVisits = sorted(dailyUniqueVisits, key=lambda x: datetime.strptime(x, '%d.%m.%Y').timestamp())
